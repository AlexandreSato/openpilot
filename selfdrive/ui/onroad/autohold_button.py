...
class AutoHoldButton(Widget):
  def __init__(self, button_width: int, button_height: int):
    super().__init__()
    self._params = Params()
    self._disabled_color: rl.Color = rl.Color(145, 155, 149, 255)
  ...
  def _render(self, rect: rl.Rectangle) -> None:
    text = "AutoHold\n      ON" if self._button_state else "AutoHold\n     OFF"
    ...
    center_y = self._rect.y + ((self._rect.height - FONT_SIZE) // 6)

    rl.draw_text_ex(self._font_semi_bold, text, rl.Vector2(center_x, center_y), FONT_SIZE, 0, color)
    rl.draw_rectangle_rounded_lines_ex(self._rect, 0.8, 30, 8, color)